...
def setup_model_selection_handler(components):
    ...
    def update_model(model_name):
        ...
        wan_param_update, kling_param_update, seedance_param_update = update_video_model_visibility(model_name)
        return wan_param_update, kling_param_update, seedance_param_update
    
    # Group 컴포넌트들을 outputs로 설정
    i2v_model_selection.change(
        fn=update_model,
        inputs=i2v_model_selection,
        outputs=[
            i2v_video_parameter['wan']['group'],
            i2v_video_parameter['kling']['group'],
            i2v_video_parameter['seedance']['group']
        ]
    )