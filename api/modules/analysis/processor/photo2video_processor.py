...
from api.modules.music.music_service import MusicService
from api.modules.prompt.dao.prompt_dao import get_prompt_db
...
# utils
from utils.gps_util import get_gps_info, get_gps_info_json
...
async def ai_photo2video_000001(
    ...
    photo2video
    """
    logger.info("분석코드 : AI-PHOTO2VIDEO-000001 시작")
    options = req_body.option
    if options == None:
        raise Exception(ANALYSIS_ERROR.AI_API_ANALYSIS_OPTION_NOT_EXIST)

    # LLM 코드 가져오기
    llm_code = await get_first_priority_llm_code(db)

    # 프롬프트 가져오기
    if os.getenv("ENV") != "local":
        # LLM 프롬프트 가져오기 (DB)
        _prompt = await get_prompt_db(
            user_id=req_body.userId,
            project_id=req_body.projectId,
            group=req_body.group,
            analyis_code=req_body.type,
            db=db,
        )

    else:
        # local 환경은 prompt 파일로 처리
        _prompt = get_prompt(
            llm_code=llm_code,
            group=req_body.group,
    ...
    logger.info(
        f"user_id: {req_body.userId}, project_id: {req_body.projectId}, analysis_code: {req_body.type}, prompt: {_prompt}"
    )
    # task_llm prompt 저장
    await update_task_llm_prompt(
        req_body=req_body,
        prompt=json.dumps(_prompt, ensure_ascii=False),
        db=db,
    )
    ...
    locations = await asyncio.gather(
        *[
            get_gps_info_json(document_file, llm_code)
            for document_file in document_files
        ]
    )

    logger.info(
        f"userId: {req_body.userId}, projectId: {req_body.projectId}, analysisCode: {req_body.type} Locations: {locations}"
    )

    for document_file in document_files:
        # 이미지 메타데이터 삭제
        await remove_image_metadata(document_file)

    # 텍스트, 비디오 프롬프트 생성
    if llm_code == LLM_CODE.LLM_CHATGPT:
        video_results = await chatgpt_photo_video(
            req_body, _prompt["prompt"], document_files, locations, db
        )
    if llm_code == LLM_CODE.LLM_GEMINI:
        video_results = await gemini_photo_video(
            req_body, _prompt["prompt"], document_files, locations, db
        )

    video_type, video_name = await get_main_video_info(db)
...
# async def ai_photo2video_000002(
#     req_body: ReqDoAnalysis,
#     upload_dir: str,
#     document_files: list,
#     db: Session,
...
#     플랜A (카테고리)
#     """
#     logger.info("분석코드 : AI-PHOTO2VIDEO-000002 플랜A (카테고리) 시작")
#     if os.getenv("ENV") != "local":
...
#         )
#     locations = await asyncio.gather(
#         *[get_gps_info_json(document_file) for document_file in document_files]
#     )
...
#     image_category = await chatgpt_image_category(req_body, _prompt, document_files, db)

#     category_context_prompt = _prompt[f"{image_category.category.name}_context"]
#     category_script_prompt = _prompt[f"{image_category.category.name}_script"]
#     category_video_prompt = _prompt[f"{image_category.category.name}_video"]

#     # 이미지 to 컨텍스트
#     image_context = await chatgpt_image_context(
#         req_body, category_context_prompt, document_files, db
#     )

#     # 컨텍스트 to 스크립트
#     texts = await chatgpt_context_script(
#         req_body, category_script_prompt, "", json.dumps(image_context), db
#     )

#     # 컨텍스트 to 비디오 프롬프트
#     video_prompts = await chatgpt_context_video(
#         req_body,
#         category_video_prompt,
#         document_files,
#         json.dumps(image_context),
#         db,
#     )

#     # video_prompts = json.loads(video_prompts)["video_prompts"]
#     raw_list = json.loads(video_prompts)["video_prompts"]
#     video_prompts = [item["video_prompt"] for item in raw_list]

#     # 비디오 생성 (kling_v2_1)
#     video_bytes = await VideoService.run_multiple_api(
#         model=API_VIDEO_MODEL.KLING_V2_1,
#         document_files=document_files,
#         video_prompts=video_prompts,
#     )
...
#     result = await result_json(json.loads(texts), video_urls)

#     return result


# async def ai_photo2video_000003(
#     req_body: ReqDoAnalysis,
#     upload_dir: str,
#     document_files: list,
#     db: Session,
...
#     플랜B (카테고리)
#     """
#     logger.info("분석타입 : AI-PHOTO2VIDEO-000003 플랜B (카테고리) 시작")
#     if os.getenv("ENV") != "local":
#         _prompt = req_body.prompt[0]["prompt"]
#     else:
#         _prompt = get_prompt(
...
#     # 이미지 to 카테고리
#     image_category = await chatgpt_image_category(req_body, _prompt, document_files, db)
#     category_prompt = _prompt[f"{image_category.category.name}_image"]

#     # 텍스트, 비디오 프롬프트 생성
#     video_results = await chatgpt_photo_video(
#         req_body, category_prompt, document_files, locations, db
#     )
...
#     result = await result_json(video_results.model_dump(), video_urls)

#     return result


# async def ai_photo2video_000004(
#     req_body: ReqDoAnalysis,
#     upload_dir: str,
#     document_files: list,
#     db: Session,
# ):
#     """플랜B (올인원)"""
#     logger.info("분석타입 : AI-PHOTO2VIDEO-000004 플랜B (올인원) 시작")
...
# async def ai_photo2video_000005(
#     req_body: ReqDoAnalysis, upload_dir: str, document_files: list, db: Session
# ):
#     """
#     bgm 생성
#     """
#     logger.info("분석코드 : AI-PHOTO2VIDEO-000005 시작")

#     music_prompt = await chatgpt_photo_music(
#         req_body=req_body, document_files=document_files, db=db
#     )

#     tags = music_prompt.music_prompt

#     await MusicService.do_create_bgm(
#         model=API_MUSIC_MODEL.ACE_STEP,
#         duration=20,
#         tags=tags,
#         upload_dir=upload_dir,
#         s3_path=req_body.analysisS3,
...
# async def ai_photo2video_000006(
#     req_body: ReqDoAnalysis, upload_dir: str, document_files: list, db: Session
# ):
#     """
#     user input prompt로 video 생성
#     """
#     logger.info("분석코드 : AI-PHOTO2VIDEO-000006 시작")
...
#     enhanced_prompt: GPTContext2Video = await chatgpt_enhance_prompt(
#         req_body=req_body,
#         _prompt=_prompt["prompt"],
#         document_files=document_files,
#         db=db,
...
#     for video_result in video_results.results:
#         video_result.video_prompt = enhanced_prompt.video_prompt

#     # 최종 리스트
#     video_prompts = [
#         video_result.video_prompt for video_result in video_results.results
#     ]
#     logger.info("video_prompts : ", video_prompts)
