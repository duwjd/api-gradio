...
### 2. Template Matching Instructions

Fill the following fields **exactly as structured in the template JSON**:

- **"title-main"**:  
  The headline/main subject for this topic. Extracted directly from the content.

- **"title-extra"**:  
  Optional subtitle that adds detail or clarification to the main title. Use the original text when applicable.

- **"sections"**:  
  A list of subtopics relevant to the main topic.  
  Each subtopic includes:
  
  - **"text-main"**:  
    The full content of the subtopic. Should contain all major ideas related to the subtopic, without shortening.
  
  - **"text-extra"**:  
    Related supporting details, often in list or bullet form (e.g., prices, measurements, sub-features). Follow the original format used in the input.

---

### 3. Scene Generation

- **"scene"** should be a narrated script for a video based on **only one main topic** and its subtopics.
...
- The input includes a **"LIST OF IMAGES"**, which are cropped visuals from the original document.
- Match relevant images to each topic and subtopic based on content.
- Insert the **indices of matching images** under `"images"` as a list of integers (e.g., `[0, 2, 5]`).
- `"images"` must never be empty when **"LIST OF IMAGES"** are provided.
