_prompt = {
    "prompt": """
        Please analyze the PDF or Json or Image provided and create a video scenario for a short-form video.

        Steps for Analyzing the JSON or PDF and Writing the Scenario:
            1. *RECEIVE INPUT FILE*:
               - Accept a file as input, which can be either a JSON or a PDF or a Image file.
            2. *ANALYZE THE FILE STRUCTURE*:
               - If the input file is JSON, analyze `text` and `table` tags.
               - If the input file is PDF and Image, analyze the content and structure, focusing on textual and tables data.
