# _prompt = {
#     "prompt": """
# - Role: An expert at recognizing handwriting in images.
#
# - Output Rules:
# 1. 'emphasisList' MUST be selected from items in 'Emphasis list'.
# 2. 'category', 'title', 'mood' MUST be selected from one of those in the 'categories', 'Title list', 'Mood list'.
# 3. 'fonttype' MUST be 1, 2, or 3 based on font size, with 1 being the largest and 3 the smallest.
# 4. ENSURE that NO double quotes (") or single quotes (') appear within any string under any circumstance.
#     a. Remove any double quotes (") or single quotes (') inside strings in the output.
#         - e.g., "Valentine's Day" -> "Valentines Day"
# 5. The output list has a single item.
# 6. Return a bounding box for each of the texts in this image in [ymin, xmin, ymax, xmax] format.
#     a. Set the bounding boxes to not overlap areas.
#     b. Set the bounding box to fit snugly into the text area.
#
# - Text grouping and segmentation rules:
# 1. Replace all alphabets with lowercase letters when comparing text with the keywords.
#     a. If double quotes (") or single quotes (') are recognized while comparing recognized text and keywords, the comparison excludes the double quotes (") or single quotes (').
#         i. First, if the text "or Lunar New Year's Day" is recognized, convert it to lowercase. -> "or lunar new year's day"
#         ii. Then, if the sentence contains double quotes (") or single quotes ('), remove them. -> "or lunar new years day"
# 2. If the recognized text is diagonal, up and down, left and right, and consists of items for a keyword, it MUST be grouped.
#     - e.g., If 'new' is above 'years' and 'years' is to the left of 'day', group them as 'new\n years day' because the surrounding text can organize the items in the keywords.
#     - e.g., If 'Black', 'Friday', 'Sale' are vertically placed, group them into 'Black\n Friday', 'Sale'
#     - e.g., "Boxing", "Day", "Sales" are vertically placed -> group them into "Boxing\n Day", "Sales"
#     - e.g., "Happy", "BirthDay", "To you" are vertically placed -> group them into "Happy\n BirthDay", "To you"
...
#     a. Recognizes text arranged in ONLY one horizontal line as a single item.
...
#     b. Split the text if it detects any items in the keyword inside the recognized text.
#         - e.g., "Happy New Year 2025 with exciting and positivity" -> "Happy New Year" and "2025" are recognized in keywords -> "Happy New Year", "2025", "with exciting and positivity".
#     c. Exception: If the items in a keywords can be organized vertically, the grouping of vertically placed text is prioritized.
#         - e.g., If "Black" and "Friday" are vertically placed, group them into "Black Friday".
# 4. Analyze the recognized texts and proceed with grouping and segmentation if they can form an item for keywords.
...
#
# - keywords:
# ["new years day", "happy new year", "새해", "2025", "lunar new years day", "설날", "구정", "valentines day", "happy valentines day", "발렌타인", "발렌타인데이", "black friday", "chiristmas", "merry christmas", "christmas", "christmas eve", "boxing day", "크리스마스", "생일축하합니다", "happy birthday", "happy birthday to you"]
#
# - Emphasis Expressions:
# 1. Underline
#     a. If the underline is recognized, add 'underline' to the 'emphasisList'.
# 2. Decorative Shape: Box, Circle -> "box"
#     a. If Box shape or Circle shape are recognized, add 'box' to the 'emphasisList'.
#
# - Emphasis list:
# ["underline", "box"]
#
# - Categories:
# ["newyear", "lunarnewyear", "valentine", "blackfriday", "christmas", "birthday", "none"]
#
# - Mood list:
# ["enterprise", "sale", "spring", "summer", "fall", "winter", "cute", "basic", "cyber", "modern"]
#
# - Output format:
# ```json
# [
...
1. If vertically aligned text items are grouped, the grouped text MUST use \n to indicate line breaks.
    ...
    - e.g., `Happy`, `Birthday` and `To You` -> `Happy\n Birthday\n To You`
2. If the vertically grouped text forms part of a keyword, ENSURE the \n formatting is preserved in the output.
3. **If the text contains line breaks, determine the bounding box as follows:**  
   1. Use the **y coordinate of the first line** as `ymin`.  
   2. Find the **longest line among all lines** and use its **x coordinate** as `xmin`.  
   3. Use the **y coordinate of the last line** as `ymax`.  
   4. Use the **end x coordinate of the longest line** as `xmax`.  

This ensures that the bounding box accurately reflects the visual alignment of the text, even if the starting x coordinates of each line differ.
