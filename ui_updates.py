...
def update_ui(analysis_code):
    ...
    if analysis_code == "AI-PHOTO2VIDEO-000001":
        updates.extend([
            ...
            gr.update(visible=False),   # llm_prompt  
            gr.update(visible=True),    # user_image
            gr.update(visible=True),    # video_generation_model
            gr.update(visible=False),   # wan_parameter
            gr.update(visible=False)    # kling_parameter
        ])
    elif analysis_code in ["AI-ASSIST-000001", "AI-ASSIST-000002"]:
        updates.extend([
            gr.update(visible=True),    # llm_model
            gr.update(visible=True),    # llm_prompt
            gr.update(visible=False, value=None),   # user_image - 파일도 초기화
            gr.update(visible=False),   # video_generation_model
            gr.update(visible=False),   # wan_parameter
            gr.update(visible=False)    # kling_parameter
        ])
    else:
        updates.extend([
            gr.update(visible=False),   # llm_model
            gr.update(visible=False),   # llm_prompt
            gr.update(visible=False, value=None),   # user_image
            gr.update(visible=False),   # video_generation_model
            gr.update(visible=False),   # wan_parameter
            gr.update(visible=False)    # kling_parameter
        ])
